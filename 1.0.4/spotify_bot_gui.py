...
class SpotifyBotGUI(QWidget):
    def __init__(self):
        ...
        self.update_manager = UpdateManager(self.config_manager.config)
        self.init_ui()
        self.worker = None
    ...
    def setup_update_timer(self):
        ...
        self.update_timer.start(24 * 60 * 60 * 1000)  # 24 часа в миллисекундах
        self.check_for_updates()  # Проверяем обновления при запуске

    def check_for_updates(self):
        if self.update_manager.update_if_available():
            QMessageBox.information(self, "Обновление", "Бот был успешно обновлен. Пожалуйста, перезапустите приложение.")
            self.close()
        else:
